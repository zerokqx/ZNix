...
## Фишки
- Hyprland (WM по умолчанию) и SwayFX с заранее настроенным Noctalia Shell (панель, бар, выводы, автозапуски); также можно включить `niri`.
...
## Структура
- `flake.nix`: входная точка; подтягивает nixpkgs 25.11, home-manager, nixvim, stylix, noctalia, firefox-nightly, flake-parts.
...
- `core/`: системные модули (boot, nixpkgs, сети, звук, графика, пользователи и т.д.) плюс `znix/` для кастомных опций.
- `configs/`: дополнительные настройки (например, `throne.nix`, `zapret.nix`).
- `home/`: home-manager конфиги для `zerok`; включает Hyprland (`home/hyprland/`), пакеты, темы, nixvim (`home/nixvim/`) и т.д.

## Кастомные опции
- Система:
  - `znix.monitors.*` (`home/znix/monitors.nix`): карта мониторов (режим/позиция) и выбор для панели Noctalia.
  - `znix.dev.ports.*` (`core/znix/ports.nix`): открывает dev TCP-порты на выбранном интерфейсе (`interface`, `tcpPorts`).
  - `znix.ai.sillytavern.*` (`core/znix/silly-tavern.nix`): включение SillyTavern и Ollama (`enable`, `ollamaEnable`, `mobileServer`, `name`).
  - `znix.hardware.fingerprint.goodix.*` (`core/znix/finger-print.nix`): включает Goodix (fprintd/TOD).
- Home:
  - `znix.browser.*` (`home/znix/browser.nix`): настройки Firefox (профиль, расширения, закладки).
  - `znix.theme.*` (`home/znix/theme.nix`): полярность темы, курсоры и иконки (пакеты и названия тем для stylix).
...
## Nixvim: Tree-sitter по умолчанию отключен
- В `home/nixvim/plugins/ui/treesitter.nix` плагин `treesitter` подключен, но его функции выключены по умолчанию:
  - `highlight.enable = false`
  - `indent.enable = false`
  - `incremental_selection.enable = false`
  - `autotag.enable = false`
- Подсветка синтаксиса сейчас идет через `vim-polyglot` (`home/nixvim/plugins/ui/polyglot.nix`), так как Tree-sitter в этой конфигурации работает медленнее.

### Как включить Tree-sitter обратно
1) Включите подсветку в `home/nixvim/plugins/ui/treesitter.nix`:
`highlight.enable = true;`
2) Отключите `vim-polyglot`:
   - удалите `./polyglot.nix` из `imports` в `home/nixvim/plugins/ui/default.nix`, либо
   - уберите `vimPlugins.vim-polyglot` из `home/nixvim/plugins/ui/polyglot.nix`.
3) Примените конфигурацию (`home-manager switch` или `sudo nixos-rebuild switch --flake .#<host>`).
